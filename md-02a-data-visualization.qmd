...
Note - In order to remove outliers from the boxplot, I wrote "outlier.shape=NA" within the geom_boxplot function. Reference - <https://www.geeksforgeeks.org/ignore-outliers-in-ggplot2-boxplot-in-r/>

Data types of the variables

1 - life expectancy - numerical discrete data

2 - continent (used in the x axis )- non-numerical categorical data

There are only 2 variables - why does the question says 3??
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: I think that the geom_bar counts the number of times that particular continent appears in the data set and then it totals it up and summarises it in the form of a chart. When I use the function geom_col on this data then nothing happens because this function (geom_col) is not doing the same thing as geom_bar in the backend.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function. Answer - Similar to geom_bar, geom_histogram is also calculating in the back end based on the specification we provide in the code - It is not data that is readily available in the table but the function is doing that extra layer of processing to create the graph. This is contrary to geom_col where the data set had a column called count which the function just plotted.

7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: Its altering the lowest, highest and the intereval in the graph.

```{r}
ggplot(gapminder_2007, aes(x=lifeExp, fill=continent))+
  geom_histogram(col="grey30",breaks=seq(30,90,5))
```
